<template>
	<div>
		<v-progress-linear v-if="fetching" color="primary" indeterminate></v-progress-linear>
		<div v-if="exercice.length>0" class="table">
							...
							<div class="inner">
								...
								<form class="quiz-body" @submit.prevent="validateUserResponse">
									<ol class="quiz-question-list">
										...
										<v-btn v-if="is_student" color="primary" type="submit">Valider</v-btn>
									</ol>
								</form>
							</div>
						</div>
		...
		<DialogReponses :correctionExerciceProps="correction_exercices" @nextEmited="getNetxtChapter()"/>
	...
	methods: {
		...
		validateUserResponse(){
			...
			if(this.responses.length==0){
				...
				axios
					.post(`${this.url}/reponseEleves/`, {"reponses":this.responses,"eleve":this.$store.state.user.current_student.id}, this.headers)
					.then((res) => {
